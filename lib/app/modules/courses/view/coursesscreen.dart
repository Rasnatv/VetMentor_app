...
// TODO: fix this import to match your actual project structure —
// it should point at the file containing `CareersScreen`.
...
class CourseListingScreen extends StatelessWidget {
  ...
  void _openCareers(BuildContext context) {
    // CareersScreen has no controller dependency in the code shared so
    // far, so a plain Get.to is enough. Add a binding here if yours needs one.
      ...
      body: Obx(() {
        ...
        if (_ctrl.isLoading.value) {
          ...
          content = ListView.separated(
            ...
            itemCount: _ctrl.courses.length + 1, // +1 for the careers CTA
            separatorBuilder: (_, __) =>
                SizedBox(height: r.spacing(AppDimens.paddingMD)),
            itemBuilder: (_, i) {
              if (i == 0) {
                return _CareersCta(r: r, onTap: () => _openCareers(context));
              }
              final course = _ctrl.courses[i - 1];
...
/// Full-width call-to-action linking the course list back to the
/// career roadmap. Shown as the first item in the list so it scrolls
/// with the courses rather than pinning above the banner.
class _CareersCta extends StatelessWidget {
  final Responsive r;
  final VoidCallback onTap;

  const _CareersCta({required this.r, required this.onTap});
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.route_outlined,
                  color: Colors.white, size: r.fontSize(18)),
            ),
            SizedBox(width: r.spacing(AppDimens.paddingMD)),
            Expanded(
              child: Column(
            ...
            Icon(Icons.arrow_forward_ios_rounded,
                size: r.fontSize(14), color: AppColors.primary),
          ],
        ),
      ),
    );
