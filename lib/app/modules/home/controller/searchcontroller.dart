...
class SearchCollegeModel {
  ...
  final String district;
  final String state;
  // ✅ removed `type` — /search-colleges doesn't return it;
  // use EnquiryController.registeredCollegeType at the call site instead.
...
class SearchController extends GetxController {
  ...
  // ✅ On iOS, if the student's stored registered type is '1', hide all
  // search results — there's no per-college `type` on this endpoint, so
  // this is the only signal we have. Android is unaffected.
  List<SearchCollegeModel> _platformVisible(List<SearchCollegeModel> list) {
    if (Platform.isIOS) {
  ...
  Future<void> search(String q) async {
    ...
    try {
      ...
      if (status == '1' && statusCode == 200 && raw is List) {
        ...
        final visible = _platformVisible(list); // ✅
        results.assignAll(visible);
        state.value = visible.isEmpty ? SearchState.empty : SearchState.success;
      } else {
        state.value = SearchState.empty;
      }
    } on DioException catch (e) {
