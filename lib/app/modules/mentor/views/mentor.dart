  ...
  Widget build(BuildContext context) {
    ...
    return NetworkAwareWrapper(
      child: Scaffold(
        ...
        appBar: VetAppBar(showBack: false, title: 'Talk to a Mentor'),
        // ── Pull-to-refresh ─────────────────────────────────────────────
        // Pulling down from the top of the page now re-runs fetchVideos(),
        // the same call the small refresh icon in the channel card uses.
        body: RefreshIndicator(
          onRefresh: controller.fetchVideos,
          color: AppColors.primary,
          child: ListView(
            // Needed so the pull gesture still works even if the content
            // is shorter than the screen height.
