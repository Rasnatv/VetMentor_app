...
//       ]);
//
//       final statesRes   = StatesResponse.fromJson(results[0].data as Map<String, dynamic>);
//       final programsRes = ProgramsResponse.fromJson(results[1].data as Map<String, dynamic>);
//
...
class ProfileController extends GetxController {
  ...
  String get _storedStudentId {
    // Read from EnquiryController memory first (already set before navigation)
    try {
      final enquiry = Get.find<EnquiryController>();
      final id = enquiry.studentId.toString();
      if (id.isNotEmpty && id != '0') return id;
    } catch (_) {}
    // Fallback: storage (used on cold app restart)
