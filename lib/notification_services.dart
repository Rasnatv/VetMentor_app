...
import 'app/core/storage/fcm _token storage.dart';
...
class NotificationService with WidgetsBindingObserver {
  ...
  /// Completes exactly once, the moment the FCM token has been fetched
  /// (or definitively failed to fetch, with null). Other code — like
  /// PushNotificationController — should `await` this instead of
  /// polling storage and guessing how long init() will take.
  final Completer<String?> _tokenCompleter = Completer<String?>();
  Future<String?> get tokenReady => _tokenCompleter.future;
  ...
  Future<void> init() async {
    ...
    _firebaseMessaging.onTokenRefresh.listen((token) {
      ...
      FcmTokenStorage.instance.setFcmToken(fcmToken: token);
    });
    ...
    await FcmTokenStorage.instance.setFcmToken(fcmToken: token);

    // Signal that the token flow is done (success or null) — this is
    // what PushNotificationController waits on.
